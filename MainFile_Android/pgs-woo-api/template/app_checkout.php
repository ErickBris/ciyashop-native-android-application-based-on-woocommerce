        ...
        foreach ( $cart_items as $values ){
            ...
            if($is_abandoned_cart){
                ...
                if(isset($device_token) && !empty($device_token)){
                    pgs_woo_api_add_abadonded_cart_meta($cart_item_key,$device_type,$device_token);
                }                            
            }
        }        
    }
...
<style>.about_paypal { display: none;}
.woocommerce .woocommerce-checkout-review-order-table .order-total td{ color: <?php echo esc_attr($app_color['primary_color'])?> !important; }
.woocommerce button, input[type="button"], input[type="submit"]{ background-color: <?php echo esc_attr($app_color['primary_color'])?> !important; }
.woocommerce .input-text:focus { border-width: 2px; border-color: <?php echo esc_attr($app_color['primary_color'])?> !important; }
.select2-container--default.select2-container--open li:hover{ background-color: <?php echo esc_attr($app_color['primary_color'])?> !important; }
.select2-container--default.select2-container--open .select2-results__option--highlighted{ background-color: <?php echo esc_attr($app_color['primary_color'])?> !important; }
</style>
<?php wp_head(); ?>
</head>
<body <?php body_class();?>>
